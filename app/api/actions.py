...
from app.core.db_helpers import get_all_actions, update_action_status, delete_action, get_action_by_id
from app.services.n8n_client import send_action_to_n8n
...
async def get_actions(
    ...
    if urgency:
        # DB stores urgency in lowercase (critical/high/medium/low)
        actions = [a for a in actions if a.get("urgency") == urgency.lower()]
...
async def update_action_status_endpoint(action_id: str, status: str):
    ...
    # If the user clicks "Send", we can pipe this to n8n
    if status == "sent":
        action_doc = await get_action_by_id(action_id)
        if action_doc:
            send_action_to_n8n(action_doc)
